...
class EuipoSearchRequest(BaseModel):
    ...
    page: int = 0
    size: int = 10  # EUIPO api sembra accettare >=10
...
async def _get_access_token() -> str:
    ...
    if r.status_code != 200:
        # ritorniamo un 502 con dettagli utili
        raise HTTPException(
            status_code=502,
    ...
    if not token:
        raise HTTPException(status_code=502, detail=f"Token response missing access_token: {j}")
